...
substitute [] t = t
substitute ((Equ (Variable x) t):xs) y = substitute xs (subst y x t) 
...
vara (AtomicRel _ tList) = concatMap var tList
...
unifya (AtomicRel p1 ts1) (AtomicRel p2 ts2)
    | p1 /= p2 = Failure  
    | otherwise = unify (zipWith Equ ts1 ts2)  
...
varcl :: Clause -> [String]
varcl (Clause at ats) = concatMap vara ats ++ vara at

-- substitution for a clause
substitutecl :: [Equ] -> Clause -> Clause
substitutecl es (Clause at ats) = Clause at (map (substitutea es) ats)
...
varngl :: Goal -> [String]
varngl g = filter (flip notElem (vargl g)) freshvarlist


-- renaming of a clause according to a goal
renclause :: Goal -> Clause -> Clause
renclause g c = substitutecl [Equ (Variable x) (Variable y) | (x,y) <- zip (varcl c) (varngl g)] c
...
evaluatenc p (Goal ats) c = concatMap (evaluatemix p c) (mixg ats)
...
evaluate p g = map (restrict (vargl g)) (evaluate p g)
...
extr :: [[Equ]] -> Int
extr (((Equ (Variable "x") t):ls1):ls2) = backp t
