...
evenoddf :: Peano -> (Bool, Bool) -- Par - (True, False), Impar - (False, True)
evenoddf = fix (\f p -> case p of
                Zero -> (True, False)
                Succ m -> let (b1, b2) = f m in (b2, b1)) -- Inversam elementele din tuplu 
...
denot Skip = fix (\f l -> l)
--- !!! Continuare de aici pt. toate keyword-urile "undefined" ! 
...
instance Functor StmtF where
    ...
    fmap eval a@(AtrEF t e) = undefined
