...
substaexp (Qid s1) s2 e = if (s1 == s2) then e else (Qid s1)
substaexp (PlusE e1 e2) s e = PlusE (substaexp e1 s e) (substaexp e2 s e)
substaexp (TimesE e1 e2) s e = TimesE (substaexp e1 s e) (substaexp e2 s e)
substaexp (DivE e1 e2) s e = DivE (substaexp e1 s e) (substaexp e2 s e)     
...
valueassn (BEX b) _ = b
valueassn (LEX e1 e2) l = if (value e1 l <= value e2 l) then True else False
valueassn (NotEX assn) l = not (valueassn assn l)
valueassn (AndEX assn1 assn2) l = (valueassn assn1 l) && (valueassn assn2 l)
valueassn (DisjInfX lassn) l = or [valueassn x l | x <- lassn]
...
substassn (LEX e1 e2) s ae = LEX (substaexp e1 s ae) (substaexp e2 s ae)
substassn (NotEX assn) s ae = NotEX (substassn assn s ae)
substassn (AndEX assn1 assn2) s ae = AndEX (substassn assn1 s ae) (substassn assn2 s ae)
substassn (DisjInfX lassn) s ae = DisjInfX [substassn x s ae | x <- lassn] -- OrEX 
...
wp :: Stmt -> Assn -> Assn
wp Skip assn = assn
wp (AtrE s e) assn = substassn assn s e
wp (Seq s1 s2) assn = wp s1 (wp s2 assn)
wp (IfE be s1 s2) assn = orx (AndEX (convassn be) (wp s1 assn)) (AndEX (NotEX (convassn be)) (wp s2 assn))
wp (WhileE be s1) assn = DisjInfX (AndEX (NotEX (convassn be)) assn : fmap (AndEX (convassn be)) (extr (wp s1 (wp (WhileE be s1) assn))))
