...
-- Gasim "int" asociat valorii "string" din tuplu
recall :: String -> [(String, Int)] -> Int
recall s [] = 0
recall s (x:xs) = if(s == fst(x))
                  then snd(x)
                  else recall s xs


-- Returnam lista modificand "int" asociat "string" gasit, cu "c"
update :: String -> Int -> [(String, Int)] -> [(String, Int)]
update s c [] = [(s, c)]
update s c (x:xs) = if(s == fst(x))
                    then update s c xs
                    else x : update s c xs
...
value (DivE e1 e2) l = div (value e1 l) (value e2 l)
...
bssos (AtrE s e) l = update s (value e l) l
bssos (Seq st1 st2) l = bssos st1 (bssos st2 l)
bssos (IfE e st1 st2) l = if(valueb e l == True)
                          then bssos st1 l
                          else bssos st2 l
bssos (WhileE e st) l | valueb e l == True = bssos (WhileE e st) (bssos st l)
                      | otherwise = l
